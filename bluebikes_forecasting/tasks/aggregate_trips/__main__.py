...
from bluebikes_forecasting.tasks.aggregate_trips import (
...
context = AggregateTripsContext(**script_config, output_data_dir=output_data_dir)

logger.info(f"Using {context.output_storage} storage: {context.output_data_dir}")
logger.info(f"Stations of interest file: {context.stations_of_interest_file}")
logger.info(f"Hourly aggregation start date: {context.hourly_start_date}")
